...
class Stabilizer:
    def __init__(self , car_detection_model_path , angle_estimation_model_path):
        ...
        try:
            ...
            print("constructor Exception - angle",e)
    ...
    def estimate_angle(self , img):
        img1 = self.angle_transforms(img)
        img1 = np.array(img1)
        img1 = np.expand_dims(img1, 0).astype(np.float32)
        return self.angle_model.run(None, {"input":img1})[0][0][0]
         

    def run(self , img_address , output_address , height_fraction = 0.45):
        try:
            img = cv2.imread(img_address)
            angle = self.estimate_angle(Image.fromarray(img)) 
            img1 = self.rotate_image(img , -angle )
            img1 = self.centerizer(img)
